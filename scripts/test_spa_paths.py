...
Drives the REAL app (server/main.py) over a REAL uvicorn server on a loopback
port. That matters: a TestClient normalizes the URL before the request is built,
which hides the percent-encoded traversal entirely - the exact vector that
survives the server's own path normalization and reaches the handler intact.
...
- a drive-absolute request path cannot escape (pathlib discards the base when
  the right-hand side has a drive, so this is not caught by a naive prefix test)
...
config.WEB_DIST = DIST          # bound by `from .config import WEB_DIST` in main
...
async def _no_autodetect():
    """Keep the test off the network and out of the user's real backends."""
    return []


main_mod.auto_detect_providers = _no_autodetect
...
def serve() -> uvicorn.Server:
    cfg = uvicorn.Config(main_mod.app, host="127.0.0.1", port=PORT, log_level="critical")
...
def get(url_path: str) -> httpx.Response:
    ...
    return httpx.get(BASE + url_path, timeout=10)
